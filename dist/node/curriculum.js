var $hPuiw$jsondiffpatch = require("jsondiffpatch");
var $hPuiw$uuid = require("uuid");
var $hPuiw$jsonschemarefparser = require("json-schema-ref-parser");
var $hPuiw$lodash = require("lodash");
...
function $57ffaf843300749d$var$envIsNode() {
    var isNode = new Function("try { return this===global; } catch(e) { return false; }");
    return isNode();
}


if ($57ffaf843300749d$var$envIsNode()) {
...
class $57ffaf843300749d$export$2e2bcd8739ae039 {
    ...
    add(schemaName, section, object) {
        ...
        if (section == 'deprecated') throw new Error('You cannot add to deprecated, use the deprecate function instead');
    ...
    deprecate(entity, replacedBy) {
        ...
        if (!currentSection) throw new Error('entity ' + entity.id + ' is not part of any schema');
        this.replace(entity.id, replacedBy);
    }
    update(section, id, diff) {
        if (section == 'deprecated') throw new Error('You cannot update deprecated entities');
        var entity = this.index.id[id];
        var clone = this.clone(entity);
        ($parcel$interopDefault($hPuiw$jsondiffpatch)).patch(clone, diff);
        ...
        if (typeof entity.unreleased == 'undefined' || !entity.unreleased) {
            if (section == 'deprecated') {
                ...
                if (Object.keys(diff).length > 1 || typeof diff.replacedBy == 'undefined') throw new Error('illegal deprecated entity update ' + id + ': ' + JSON.stringify(diff));
            }
            if (clone.id == entity.id) clone.id = this.uuid();
            this.add(section, clone);
            this.replace(entity.id, clone.id);
        } else {
            // no need to deprecate entity, just update its contents
            if (clone.id != entity.id) throw new Error('update cannot change entity id');
            entity = ($parcel$interopDefault($hPuiw$jsondiffpatch)).patch(entity, diff);
        }
     ...
     */ replace(id, newId) {
        ...
        if (!Array.isArray(this.schema[schemaName][section])) throw new Error(section + ' is not part of schema ' + schemaName);
        if (newId) var newObject = this.index.id[newId];
        if (!oldObject) throw new Error('Could not find entity with id ' + id + ' to replace');
        // if oldObject was released, deprecate it and set replaces/replacedBy references
        if (!oldObject.unreleased) {
            if (newObject) {
                if (!newObject.replaces) newObject.replaces = [];
                newObject.replaces.push(id);
            }
            if (!oldObject.replacedBy) oldObject.replacedBy = [];
            if (newId) oldObject.replacedBy.push(newId);
        }
        if (!oldObject.types) oldObject.types = [];
        oldObject.types.push(section);
        oldObject.types = [
            ...new Set(oldObject.types)
        ];
        ...
        if (!oldObject.unreleased) // add oldObject to deprecated list
        {
            if (this.index.type[oldObject.id] != 'deprecated') {
                this.data.deprecated.push(oldObject);
                if (!this.schema[schemaName].deprecated) throw new Error('schema ' + schemaName + ' missing deprecated');
                if (!Array.isArray(this.schema[schemaName].deprecated)) throw new Error('schema ' + schemaName + ' deprecated is not an array');
        ...
        if (refs) refs.forEach((id)=>{
            var refOb = this.index.id[id];
            if (refOb.deleted || this.index.type[refOb.id] === 'deprecated') return; // this is/will be deprecated anyway, so changes here won't have any effect
            if (!refOb.unreleased) // unreleased entities don't need to change their id if properties change
            // so only mark released entities as dirty
            refOb.dirty = true;
            if (Array.isArray(refOb[prop])) {
                if (newId) refOb[prop] = refOb[prop].map((refId)=>{
                    if (refId === oldObject.id) return newId;
                    return refId;
                });
                else refOb[prop] = refOb[prop].filter((refId)=>refId !== oldObject.id
                );
            } else if (typeof refOb[prop] === 'string' || refOb[prop] instanceof String) {
                if (newId) refOb[prop] = newId;
                else delete refOb[prop];
            } else throw new Error('Unexpected property type for ' + prop + ' ( ' + typeof refOb[prop] + ') ' + refOb.id);
        });
    }
    ...
    parseSchema(schema1) {
        var resolveAllOf = function() {
            ...
            var customizer = function(objValue, srcValue) {
                if (($parcel$interopDefault($hPuiw$lodash)).isArray(objValue)) return ($parcel$interopDefault($hPuiw$lodash)).union(objValue, srcValue);
                return;
            };
                ...
                if (inputSpec && typeof inputSpec === 'object') {
                    if (Object.keys(inputSpec).length > 0) {
                        if (inputSpec.allOf) {
                            ...
                            var nested = ($parcel$interopDefault($hPuiw$lodash)).mergeWith.apply(($parcel$interopDefault($hPuiw$lodash)), [
                                {
                                }
                            ].concat(allOf, [
                                customizer
                            ]));
                            inputSpec = ($parcel$interopDefault($hPuiw$lodash)).defaultsDeep(inputSpec, nested, customizer);
                        }
        ...
        return ($parcel$interopDefault($hPuiw$jsonschemarefparser)).dereference(schema1).then(function(schema) {
    ...
    async loadData(schemaName) {
                ...
                switch(this.sources[schemaName].method){
                    ...
                    case 'file':
                        ...
                        return new Promise((resolve, reject)=>{
                            fs.readFile(baseDir + schema.properties[propertyName]['#file'], 'utf8', (err, data)=>{
                                if (err) reject(err);
                                else resolve(data);
                            });
        ...
        return Promise.allSettled(Object.values(data1)).then((results)=>{
    ...
    async loadContextFromGithub(schemaName, repository, owner, branchName, authToken = null) {
        ...
        var getFile = function(filename, list1) {
            ...
            let entry = list1.data.tree.filter(function(file) {
                return file.path == node;
            }).pop();
            let hash = entry.sha;
            if (nodes.length) return octokit.rest.git.getTree({
                owner: owner,
                repo: repository,
                tree_sha: hash
            }).then((list)=>getFile(nodes.join('/'), list)
            );
            else return octokit.rest.git.getBlob({
                owner: owner,
                repo: repository,
                file_sha: hash
            }).then((data)=>data.data
    ...
    exportFiles(schema, schemaName, dir = '') {
        ...
        properties.forEach(function(propertyName) {
            ...
            } else console.warning('skipping export of ' + propertyName + ' - no source file defined');
        });
    }
    clone(object) {
    ...
    getDirty() {
        ...
        Object.keys(this.index.id).forEach((id)=>{
            if (this.index.id[id].dirty && !this.index.id[id].unreleased) dirty.push(this.index.id[id]);
        });
        return dirty;
    }
    parseSchema(schema2) {
        var resolveAllOf = (()=>{
            // from https://github.com/mokkabonna/json-schema-merge-allof
            var customizer = function(objValue, srcValue) {
                if (Array.isArray(objValue)) return ($parcel$interopDefault($hPuiw$lodash)).union(objValue, srcValue);
                return;
            };
            return (inputSpec)=>{
                if (inputSpec && typeof inputSpec === 'object') {
        ...
        return ($parcel$interopDefault($hPuiw$jsonschemarefparser)).dereference(schema2).then(function(schema) {
            return resolveAllOf(schema);
        });
    }
}
